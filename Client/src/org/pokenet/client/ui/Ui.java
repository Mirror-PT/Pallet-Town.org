...
import org.pokenet.client.ui.frames.Pokedex;
...
import org.pokenet.client.ui.frames.newsWindow;
...
public class Ui extends Frame {
    ...
    private newsWindow m_newsPage;
    private Pokedex pokemondex;
	...
	public Ui(Display display) {
		...
		m_moneyLabel.setFont(GameClient.getFontChatLarge());
	...
	public void startButtons(){
        ...
        m_buttons[8] = HUDButtonFactory.getButton("news");
        m_buttons[8].addActionListener(new ActionListener() {
        	public void actionPerformed(ActionEvent e) {
        		toggleNews();
        	}
        });
        m_buttons[8].setToolTipText("News");
    ...
    public void toggleNews(){
    	if(m_newsPage != null) {
    		getDisplay().remove(m_newsPage);
    		hideHUDElements();
    	} else {
    		hideHUDElements();
    		m_newsPage = new newsWindow();
    		m_newsPage.setLocation(m_buttons[8].getX(), 67 - getTitleBar().getHeight());
    		getDisplay().add(m_newsPage);
    	}
    }
    
    
    public void openPokedex(){
    	pokemondex = new Pokedex();
    }
    /**
