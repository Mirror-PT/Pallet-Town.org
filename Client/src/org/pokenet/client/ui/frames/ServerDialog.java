		...
		try {
			...
			m_servers[0].addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					GameClient.setHost(GameClient.serverIP);
				}
			});
			this.add(m_servers[0]);
			
			//Below is another option for a server selection. Isn't really needed, but, bleh <3 On default it's hidden
			m_servers[1] = new Button("PKMN-NEXUS World Zone_2");
			m_servers[1].setSize(0, 0);
			m_servers[1].setLocation(0, 0);
			m_servers[1].setVisible(true);
			m_servers[1].addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					GameClient.setHost("127.0.0.1");
				}
			});
