...
class ChatWidget extends Container{
    ...
    public void scroll(int indexMod){
    	...
    	for (int i = 0; i < m_maxLines; i++){
    		...
    		m_shownChat.get(i).setFont(GameClient.getFontChatSmall());
    		...
    		try {
    			...
    			// Pink text for whispers 
    			if (m_wrappedText.get(m_scrollIndex + i).charAt(0) == '|'){
    				m_shownChat.get(i).setForeground(Color.pink);
    				m_wrappedText.set(m_scrollIndex + i, m_wrappedText.get(m_scrollIndex + i).substring(1));
    			}
    		...
    		y += GameClient.getFontChatSmall().getHeight("X");
    	} 
    }
	...
	public void wrap(){
		...
		if (m_contents.size() != 0) {
			for (int i = 0; i < m_contents.size(); i++) {
				try{
					if (GameClient.getFontChatSmall().getWidth(m_contents.get(i)) <= getWidth()){
						...
						if (loopLine.charAt(0) == '*')
							...
							messageType = '|';
								...
								.getUsername())))
							messageType = '!';
						while (GameClient.getFontChatSmall().getWidth(loopLine) > getWidth()){
							int linesToDrop = 1;
							while (GameClient.getFontChatSmall().getWidth(loopList.get(
    ...
    public void setSize(float width, float height){
		...
		m_maxLines = (int)(getHeight() / GameClient.getFontChatSmall().getHeight("X"));
		wrap();
		layoutScrollButtons();
		scroll(0);
    }
