...
public class ChatDialog extends Frame {
	...
	private Label m_backdrop;
	private String m_path = "res/ui/";
	...
	public ChatDialog() {
		...
		loadImages();
		m_backdrop.setLocation(m_chatWidget.getX(), m_chatWidget.getY());
		add(m_backdrop);
		...
		m_inputBox.setBackground(new Color(0,0,0));
		initGUI();
		addChat("Local", false);
		addAnnouncement("Welcome to Kanto!");
	}
	/**
	...
	public void loadImages(){
		...
		try {
			m_backdrop = new Label(new Image( m_path + "chatbox.png", false));
		} catch (SlickException e) {}
		m_backdrop.setSize(400, 185);
	}
	/**
	...
	public void addWhisperLine(String chat, String line) {
		if (m_availableChats.containsKey(chat)) {
			for (String s : m_availableChats.keySet())
				m_availableChats.get(s).add('|' + " " + line);
