...
public class PlayerInfoDialog extends Frame {
	...
	private void initGUI() {
		...
		// Trainer data labels
		m_trainerEXP = new Label("Trainer Lvl:          "
		...
		m_kantoLbl.setY(m_trainerEXP.getY()
				+ m_trainerEXP.getHeight() + 4);
		...
		if (ALL_REGIONS) {
			...
			setSize(160 + 2 + maxLblWidth, m_kanto[0].getY()
					+ m_kanto[0].getHeight() + getTitleBar().getHeight() + 2);
		}
	...
	public void loadImages() {
		...
		for (int i = 0; i < 8; i++) {
			try {
				...
				m_kanto[i].setY(m_trainerEXP.getY()
						+ m_trainerEXP.getHeight() + 2);
