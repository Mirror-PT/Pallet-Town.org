		...
		synchronized (m_mapMatrix.getPlayers()) {
			...
			while (it.hasNext()) {
				...
				if (p != null && p.getSprite() != 0
					...
					g.resetTransform();
					// Draw player names
					if(!p.getUsername().equalsIgnoreCase("!NPC!") && !p.isOurPlayer())
					{
						
						if(p.getUsername().equals("LordUsagi")){
							Color old = g.getColor();
							g.setColor(new Color(0, 0, 0, 85));
							g.fillRect(m_xOffset
									+ (p.getX() - (g.getFont()
											.getWidth(" (A) " + p.getUsername()) / 2)) + 16,
									m_yOffset + p.getY() - 36, g.getFont().getWidth("(A) " + p.getUsername()), g.getFont().getHeight("(A) " + p.getUsername()));
							g.setColor(new Color(255,215,0));
							g.drawString("(A) " + p.getUsername(), m_xOffset
									+ (p.getX() - (g.getFont()
											.getWidth("(A) " + p.getUsername()) / 2)) + 16,
									m_yOffset + p.getY() - 36);
							g.setColor(old);
							...
							g.fillRect(m_xOffset
									...
									m_yOffset + p.getY() - 36, g.getFont().getWidth(p.getUsername()), g.getFont().getHeight(" " + p.getUsername()));
							g.setColor(old);
