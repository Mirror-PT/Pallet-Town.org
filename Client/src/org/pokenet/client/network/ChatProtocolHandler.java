...
public class ChatProtocolHandler extends IoHandlerAdapter {
	...
	public void messageReceived(IoSession session, Object m) throws Exception {
		...
		case 'F':
			...
			case 'o': //Logged on
				GameClient.getInstance().getUi().getFriendsList().setFriendOnline(message.substring(1), true);
				break;
			case 'n': //Logged off
				GameClient.getInstance().getUi().getFriendsList().setFriendOnline(message.substring(1), false);
				break;
		case 'a':
