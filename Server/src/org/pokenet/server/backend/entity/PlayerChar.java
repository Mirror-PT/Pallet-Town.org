...
public class PlayerChar extends Char implements Battleable, Tradeable {
	...
	private MySqlManager m = new MySqlManager();
	/*
	...
	public void addFriend(String username) {
		...
		if(m_friends.size() < 10) {
			...
			if(m.connect(GameServer.getDatabaseHost(), 
					...
					GameServer.getDatabasePassword())) {
				m.selectDatabase(GameServer.getDatabaseName());
				m.query("INSERT INTO friends_data VALUES ('" + 
						this.getName() + "','" + username + "',1)");
				m.close();
			}
			m_tcpSession.write("Fa" + username);
			m_tcpSession.write("Fo" + username);
		}
	}
		...
		for(int i = 0; i < m_friends.size(); i++) {
			if(m_friends.get(i).equalsIgnoreCase(username)) {
				...
				if(m.connect(GameServer.getDatabaseHost(), 
						...
						GameServer.getDatabasePassword())) {
					m.selectDatabase(GameServer.getDatabaseName());
					m.query("DELETE FROM friends_data WHERE playerName='" + 
							this.getName() + "' AND friendName='" + username + "'");
					m.close();
				}
	...
	public void justAddItem(int id, int quantity){
		if(m_bag.hasSpace(id)) {
			m_bag.addItem(id, quantity);
			TcpProtocolHandler.writeMessage(m_tcpSession, new ItemMessage(true, 
					GameServer.getServiceManager().getItemDatabase().getItem(id).getId(), quantity));
		} else {
			m_tcpSession.write("qYou do not have enough bag space for that item!");
		}
	}
}
